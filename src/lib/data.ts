...
export class HttpProvider implements ExplorerProvider {
  ...
  // Scan thêm 1 block nữa để biết có còn data không
  while (collected.length < offset + pageSize + 1 && h >= 0 && scanned < maxScanBlocks) {
    const hash = await api.rpc.chain.getBlockHash(h);
    const items = await this.mapExtrinsicsForBlock(h, hash.toHex());
  ...
  // Nếu còn data SAU pageSize hiện tại → có next page
  const hasMore = collected.length > offset + pageSize || (h >= 0 && scanned < maxScanBlocks);
