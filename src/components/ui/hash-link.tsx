...
export function HashLink({ hash, type = 'tx', truncate = true, showCopy = true, className = "" }: HashLinkProps) {
    const displayHash = truncate && hash.length > 20 ? `${hash.slice(0, 10)}...${hash.slice(-10)}` : hash
    const href = type === 'tx' ? `/tx/${hash}` : `/block/${hash}`
